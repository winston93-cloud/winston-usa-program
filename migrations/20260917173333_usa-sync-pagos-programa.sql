-- Sincroniza alumnos del Programa USA desde pago_detalle (conceptos 23/24/25),
-- misma lógica de referencia 12 dígitos que servicios_admin (parsearReferenciaPago).

CREATE UNIQUE INDEX IF NOT EXISTS usa_programa_alumno_ref_uq
  ON public.usa_programa_alumno (alumno_ref)
  WHERE alumno_ref IS NOT NULL;
...
REVOKE ALL ON FUNCTION public.usa_sync_pagos_programa(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.usa_sync_pagos_programa(integer) TO anon, authenticated;

COMMENT ON FUNCTION public.usa_sync_pagos_programa(integer) IS
  'Upsert usa_programa_alumno desde pagos 23/24/25 del ciclo (ref 12 dígitos).';
